...
if (!projectIdArg || (!domainArg && !autoDetect)) {
  console.error("Usage: bun link-by-domain.ts --project-id=123 --domain=example.com")
...
if (autoDetect) {
  const estimates = db
    .query<{ contractor: string }, [number]>("SELECT DISTINCT contractor FROM estimates WHERE project_id = ? AND contractor IS NOT NULL")
    .all(projectId)

  console.log(`Contractors: ${estimates.map((e) => e.contractor).join(", ")}`)

  for (const est of estimates) {
    const normalizedContractor = est.contractor.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 10)
