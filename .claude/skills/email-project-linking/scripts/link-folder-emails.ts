...
if (!folderName) {
  console.error("Usage: bun link-folder-emails.ts <folder-name> [--project-id=123]")
  process.exit(1)
}
...
if (!projectId) {
  const alias = db
    .query<{ project_id: number }, [string]>("SELECT project_id FROM project_aliases WHERE alias = ?")
    .get(folderName)
  ...
  if (alias) {
    ...
    console.error(`No alias found for folder "${folderName}". Add one first or use --project-id.`)
    process.exit(1)
  }
}

const project = db.query<{ name: string }, [number]>("SELECT name FROM projects WHERE id = ?").get(projectId)
...
const activeFolder = projectsFolder?.children?.find((f) => f.displayName === "Active")
const folder = activeFolder?.children?.find(
  (f) => f.displayName === folderName || f.displayName.trim() === folderName.trim()
)
...
const graphClient = client as { client: { api: (path: string) => { top: (n: number) => { select: (fields: string) => { get: () => Promise<{ value: Array<{ internetMessageId: string; conversationId: string; subject: string }> }> } } } } }
...
for (const msg of messages) {
  ...
  if (result.changes > 0) {
    ...
    const existing = db
      .query<{ project_id: number | null }, [string]>("SELECT project_id FROM emails WHERE internet_message_id = ?")
