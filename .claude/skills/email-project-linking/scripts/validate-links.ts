...
type EmailRow = { id: number; subject: string; from_email: string; from_domain: string | null }
type CountRow = { c: number }
type DomainCountRow = { from_domain: string | null; c: number }

function validateProject(projectId: number): { pass: boolean; issues: string[] } {
  const issues: string[] = []
  ...
  const aliases = db
    .query<{ alias: string }, [number]>("SELECT alias FROM project_aliases WHERE project_id = ?")
    .all(projectId)

  for (const a of aliases) {
    const aliasKeywords = a.alias.toLowerCase().split(/[^a-z0-9]+/).filter((w) => w.length > 3)
  ...
  const internalCount = domainCounts.find((d) => d.from_domain === internalDomain)?.c || 0
  const externalDomains = domainCounts.filter((d) => d.from_domain !== internalDomain)
...
if (validateAll) {
  ...
  for (const row of projectsWithEmails) {
    ...
    const emailCount = db
      .query<CountRow, [number]>("SELECT COUNT(*) as c FROM emails WHERE project_id = ?")
      .get(row.project_id)
