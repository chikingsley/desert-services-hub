...
Output: A fully functional matching module at `services/contract/matching/` with types, scorer, matcher, and storage components.
</objective>
...
<task type="auto">
  <name>Task 1: Create types and scorer module</name>
  <files>
    services/contract/matching/types.ts
    services/contract/matching/scorer.ts
  </files>
...
<task type="auto">
  <name>Task 2: Create matcher and storage modules</name>
  <files>
    services/contract/matching/matcher.ts
    services/contract/matching/storage.ts
    lib/db/index.ts
...
<task type="auto">
  <name>Task 3: Add unit tests for matcher</name>
  <files>
    services/contract/matching/matcher.test.ts
  </files>
  <action>
Create a unit test file that verifies the matching logic works correctly.

**matcher.test.ts**:
```typescript
import { describe, expect, it } from "bun:test";
import { calculateMatchScore, HIGH_CONFIDENCE_THRESHOLD, MIN_CONFIDENCE_THRESHOLD } from "./scorer";

describe("calculateMatchScore", () => {
  it("returns high score for exact project name match", () => {
    ...
    expect(result.projectScore).toBe(1);
    expect(result.contractorScore).toBe(1);
    expect(result.combinedScore).toBe(1);
  });

  it("returns partial score for partial project name match", () => {
    const result = calculateMatchScore(
    ...
    // "Paradise Valley" is contained in search term
    expect(result.projectScore).toBeGreaterThan(0.5);
    expect(result.combinedScore).toBeGreaterThan(MIN_CONFIDENCE_THRESHOLD);
  });

  it("handles missing contractor in estimate", () => {
    const result = calculateMatchScore(
      "Test Project",
      "Test Contractor",
      "Test Project",
      null // No contractor in estimate
    );
    // Should use project score only
    expect(result.contractorScore).toBe(0);
    expect(result.combinedScore).toBe(result.projectScore);
  });

  it("weights project name higher than contractor", () => {
    // Good project match, bad contractor match
    ...
    // Bad project match, good contractor match
    ...
    // Project match should win (60% weight vs 40%)
    expect(result1.combinedScore).toBeGreaterThan(result2.combinedScore);
  });
});

describe("thresholds", () => {
  it("HIGH_CONFIDENCE_THRESHOLD is 0.8", () => {
    expect(HIGH_CONFIDENCE_THRESHOLD).toBe(0.8);
  });

  it("MIN_CONFIDENCE_THRESHOLD is 0.3", () => {
    expect(MIN_CONFIDENCE_THRESHOLD).toBe(0.3);
  });
});
```

The tests verify the scoring logic without making actual Monday API calls. Use `bun test` conventions (describe/it/expect).
  </action>
  <verify>
    `bun test services/contract/matching/matcher.test.ts` passes all tests
  </verify>
  <done>
    - Unit tests exist for calculateMatchScore function
    - Tests verify exact matches, partial matches, missing contractor handling, and weight behavior
    - All tests pass
  </done>
</task>
...
1. `bunx ultracite check services/contract/matching/` - No lint errors
2. `bun x tsc --noEmit services/contract/matching/*.ts` - No type errors
3. `bun test services/contract/matching/` - All tests pass
4. Database has contract_matches table: `bun -e "import { db } from './lib/db'; console.log('table exists:', !!db.query('SELECT name FROM sqlite_master WHERE type=\\'table\\' AND name=\\'contract_matches\\'').get())"`
</verification>
