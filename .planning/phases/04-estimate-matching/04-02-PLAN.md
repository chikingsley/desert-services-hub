...
must_haves:
  ...
  key_links:
    ...
    - from: "services/contract/matching/link.ts"
      ...
      via: "updateItem call to link contract in Monday"
      pattern: "updateItem"
---
...
- Runs the matcher
- Stores auto-matches in SQLite AND updates Monday.com (MATCH-05)
- Provides helpers for manual selection workflow (also updates Monday)
- Formats candidates for human review
  ...
  - Auto-matches are stored in SQLite AND linked in Monday.com (MATCH-05)
  - Manual selections also update Monday.com
...
1. Add a new function `runEstimateMatching` after `runClaudeExtraction`:
...
1. Update the `processContract` function to call matching after extraction. Add after the Claude extraction try/catch block (around line 120):
...
- Auto-matched contracts (>0.8 confidence) stored in SQLite AND linked in Monday.com
- Low-confidence matches log candidates and suggest manual selection
- Manual selections also update Monday.com (MATCH-05)
- Match failures don't crash the pipeline
