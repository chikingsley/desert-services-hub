...
must_haves:
  ...
  artifacts:
    - path: "services/contract/matching/link.ts"
      ...
      exports: ["linkContractToEstimate", "processContractMatch"]
    - path: "services/contract/pipeline/index.ts"
      provides: "Pipeline with matching integration"
      contains: "findEstimateMatch"
  key_links:
    - from: "services/contract/pipeline/index.ts"
      to: "services/contract/matching/matcher.ts"
      via: "findEstimateMatch import"
      pattern: "import.*findEstimateMatch.*from.*matching"
    - from: "services/contract/matching/link.ts"
      to: "services/monday/client.ts"
      via: "updateItem for Monday API"
      pattern: "updateItem"
    - from: "services/contract/matching/link.ts"
...
/**
 * Extracted contract info relevant for matching.
 * These fields come from the contractInfo agent extraction.
 */
type ContractInfo = {
...
This module:
- Gets extracted contract data from Phase 3
- Runs the matcher
...
```typescript
import { processContractMatch, formatCandidatesForSelection } from "../matching/link";
import type { MatchResult } from "../matching/types";
import { findEstimateMatch } from "../matching/matcher";
```
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    1. Matching module with multi-field scoring (Plan 01)
    2. Link module that processes matches and stores results
    3. Pipeline integration that auto-runs matching after extraction
  </what-built>
  <how-to-verify>
