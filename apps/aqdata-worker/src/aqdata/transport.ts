import { BASE_URL, DISCLAIMER, HOME, PAGES } from "./constants";
...
export class AQSessionTransport {
  ...
  protected async fetchWithSession(url: string): Promise<Response> {
    let currentUrl = url;
    ...
    throw new Error(`Too many redirects while fetching session URL: ${url}`);
  }
  ...
  protected buildPostUrl(path: string): string {
