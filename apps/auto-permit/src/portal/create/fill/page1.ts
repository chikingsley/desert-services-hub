  ...
  try {
    ...
    if (mode === "full") {
      ...
      if (data.subsidiary !== undefined) {
        if (data.subsidiary.isSubsidiary) {
          ...
          const parentEntityType = data.subsidiary.parentEntityType;
          const parentName = data.subsidiary.parentName;
    ...
    if (data.propertyOwner !== undefined) {
      if (data.propertyOwner.isDifferent) {
        ...
        const entityType = data.propertyOwner.entityType;
    ...
    console.log(`  fillPage1 failed: ${e}`);
    return false;
  }
}
