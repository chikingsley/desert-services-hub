...
export async function searchLocation(
  ...
  const cleanParcel = parcelNumber?.replace(/[-\s]/g, "") || null;
  ...
  if (cleanParcel) {
    ...
    if (typed) {
      ...
      const clicked = await esriFrame
        .evaluate((): { success: boolean; resultText?: string } => {
          ...
          if (suggestions.length > 0) {
            ...
            first.click();
            return { success: true, resultText: first.textContent?.trim() };
          }
          ...
          if (input) {
            ...
            return { success: true, resultText: "(pressed Enter)" };
          }
      ...
      if (clicked.success) {
        ...
        await sleep(2000);
        return { success: true, searchedBy: "parcel" };
      }
    }
  }
  ...
  if (address) {
    ...
    if (typed) {
      ...
      if (clicked.success) {
        console.log(`    ✓ Clicked result: ${clicked.resultText ?? "unknown"}`);
        await sleep(2000);
        return { success: true, searchedBy: "address" };
      }
    }
  }

  console.log("    ✗ No parcel or address to search");
  return { success: false, searchedBy: null };
}
  ...
  const openedGallery = await esriFrame
    .evaluate(
      ({ primary, alt }): { success: boolean; method?: string } => {
        ...
        if (btn) {
          (btn as HTMLElement).click();
          return { success: true, method: "boxController" };
        }

        // Look for any basemap-related button
        const basemapBtns = Array.from(
    ...
    .evaluate(
      ({
        ...
        if (option) {
          ...
          return { success: true, selectedTitle: "County Aerial" };
        }
        ...
        if (option) {
          (option as HTMLElement).click();
          return { success: true, selectedTitle: "Imagery" };
        }
        ...
        if (option) {
          (option as HTMLElement).click();
          return { success: true, selectedTitle: "Aerial" };
        }
  ...
  const activated = await esriFrame
    .evaluate((selector) => {
      ...
      const editBtns = Array.from(
        document.querySelectorAll('[id*="Edit"], [class*="edit"], .sketchtool')
      );
  ...
  const result = await frame
    .evaluate(
      ({ lat, lng, zoom }) => {
        ...
        return { success: false, error: "no centerAndZoom" };
      },
      { lat, lng, zoom: zoomLevel }
    )
    .catch(() => ({ success: false, error: "exception" }));
...
export async function zoomToPolygonExtent(
  ...
  const result = await frame
    ...
    .catch((e) => ({ success: false, error: `exception: ${e}` }));
  ...
  const result = await frame.evaluate(
    ({ elementId }) => {
      ...
      const editor = dijit?.registry?.byId?.("esri_dijit_editing_Editor_0") as
        ...
        | undefined;
      if (!editor) {
        return { success: false, error: "no editor after click" };
      }
      ...
      ({ label }) => {
        ...
        for (const tpick of tpicks) {
          ...
          if (text.toLowerCase().includes(label.toLowerCase())) {
            return { found: true, id: tpick.id, text: text.substring(0, 40) };
          }
        }
        return { found: false };
      },
