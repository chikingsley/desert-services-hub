...
export async function handleNewAppPopup(
  ...
  try {
    ...
    console.log(`  Popup handler failed: ${e}`);
    return false;
  }
}
...
export async function handleNewAppPopupFresh(
  ...
  try {
    ...
    await selectCompanyInPopup(popupPage, {
      ...
      copyFromApp: "", // Not used since we're not copying
    });
    await sleep(SETTLE_MS);
    ...
    console.log(`  Fresh popup handler failed: ${e}`);
    return false;
  }
}
...
export async function handleReviseAppPopup(
  ...
  try {
    ...
    console.log(`  Revise popup handler failed: ${e}`);
    return false;
  }
}
