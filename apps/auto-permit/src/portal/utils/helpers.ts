      ...
      .evaluate(
        (sels) => {
          ...
          const stillOnList =
            ...
            document.body?.innerText?.includes("Draft Dust Applications");
...
export async function clickApplicationWithRetry(
  ...
  if (index >= count) {
    ...
    return { success: false, appId: "", strategy: null, attempts: 0 };
  }
  ...
  for (const strategy of strategies) {
    ...
    try {
      ...
      if (navigated) {
        ...
        return { success: true, appId, strategy: strategy.name, attempts };
      }
    } catch (err) {
      log(`Strategy ${strategy.name} threw: ${err}`);
    }
  }

  log(`All strategies failed after ${attempts} attempts`);
  return { success: false, appId, strategy: null, attempts };
}
...
export async function triggerLazyLoad(page: Page): Promise<void> {
  ...
  for (let i = 0; i < maxScrollAttempts; i++) {
    ...
    const atBottom = await page.evaluate((pos) => {
      return pos >= document.body.scrollHeight - window.innerHeight;
    }, scrollPos);
  ...
  return await page.evaluate(
    ({ regexStr, containerSel }) => {
      ...
      return links.map((link, index) => {
        ...
        return { id, index, projectName, companyName };
      });
    },
    {
