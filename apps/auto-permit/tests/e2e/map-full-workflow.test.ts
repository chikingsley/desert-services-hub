    ...
    .evaluate(
      ({ lat, lng, zoom }) => {
        ...
        if (map.centerAndZoom) {
          ...
          return { success: true, method: "centerAndZoom" };
        }
        ...
        if (map.centerAt) {
          ...
          return { success: true, method: "centerAt+setZoom" };
        }

        return { success: false, error: "no center methods" };
      },
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const result = await frame
      .evaluate(() => {
        ...
        return { found: false, clicked: false, visibleContainers };
      })
...
async function selectTemplateByLabel(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const result = await frame
      .evaluate(
        ({ label }) => {
          ...
          const availableTemplates = Array.from(tpicks).map((e) => ({
            id: e.id,
            text: e.textContent?.trim().substring(0, 30),
          }));
          return { found: false, availableTemplates };
        },
        { label }
      )
      .catch(() => ({
    ...
    if (attempt === 1) {
      const templates = (
        result as { availableTemplates: Array<{ id: string; text: string }> }
      ).availableTemplates;
  ...
  test(
    ...
    async () => {
      ...
      if (!panned) {
        ...
        const searchAddr =
          parcelData.address?.replace(/\s+/g, " ").trim() || TEST_ADDRESS;
