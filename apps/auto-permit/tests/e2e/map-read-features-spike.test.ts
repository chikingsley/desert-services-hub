...
async function readMapFeatures(frame: Frame): Promise<MapFeaturesResult> {
  ...
  const result = await frame
    .evaluate(() => {
      ...
      if (!map) {
        return { success: false, error: "Map not found", features: [] };
      }
      ...
      const _layers = map._layers as
        ...
        | undefined;
      if (!_layers) {
        return { success: false, error: "No _layers found", features: [] };
      }

      const layerKeys = Object.keys(_layers);
      const layerStats: Record<string, number> = {};
      const rawGraphicsDebug: Array<{
        layerId: string;
  ...
  if (!result.success) {
    ...
    return { success: false, features: [], error: result.error };
  }

  // Convert Web Mercator to lat/lng
  const rawFeatures = result.features as Array<{
    type: string;
    coordinates: Array<{ x: number; y: number }>;
    attributes?: Record<string, unknown>;
    source: string;
...
async function waitForFeatureData(
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    if (totalFeatureGraphics > 0) {
      ...
      return { loaded: true, layerCounts: counts };
    }
  ...
  return { loaded: false, layerCounts: {} };
}
...
async function exploreMapInternals(frame: Frame): Promise<void> {
  ...
  const internals = await frame.evaluate(() => {
    ...
    const layerInfoVia_layers: Array<{
      id: string;
      type: string;
...
async function exploreMapStructure(frame: Frame): Promise<void> {
  ...
  const structure = await frame.evaluate(() => {
    ...
    const editor = dijit?.registry?.byId?.("esri_dijit_editing_Editor_0");
    if (!editor) {
      return { relevantWidgets, error: "No editor" };
    }
    ...
    if (!map) {
      return { relevantWidgets, error: "No map" };
    }
    ...
    return { relevantWidgets, graphicsLayerIds, layerIds, layerDetails };
  });
    ...
    async () => {
      ...
      for (let i = 0; i < allFrames.length; i++) {
        ...
        try {
          ...
          const hasEditor = await frame
            ...
            .catch(() => false);
          console.log(
            `    Frame ${i}: ${url.substring(0, 80)}... (dijit: ${hasEditor})`
          );
      ...
      if (frame1) {
        ...
        const frameInfo = await frame1
          .evaluate(() => {
            const iframes = document.querySelectorAll("iframe");
            const iframeData = Array.from(iframes).map((f) => ({
              id: f.id,
              src: f.src,
              width: f.width,
              height: f.height,
    ...
    async () => {
      ...
      const featureGeom = await esriFrame.evaluate(async () => {
        ...
        try {
          ...
          return { error: "No polygon found", data };
        } catch (e) {
