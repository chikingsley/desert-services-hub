// biome-ignore lint/nursery/noExcessiveLinesPerFile: BuildingConnected auth process control and VNC clipboard bridge are intentionally co-located.
import { type ChildProcessWithoutNullStreams, spawn } from "node:child_process";
import { stat } from "node:fs/promises";

const LOG = "[buildingconnected-auth-api]";
...
async function getStatusPayload(
  ...
  const viewport = parseBuildingConnectedVncResolution(
    process.env.BACKGROUND_JOBS_VNC_RESOLUTION
  );
