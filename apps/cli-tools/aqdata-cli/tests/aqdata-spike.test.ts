...
describeSuite("AQ Data Spike", () => {
  ...
  test("3. navigate to Dust Application Search", async () => {
    ...
    console.log("  [3] Dust Application Search loaded, page:", client.currentPage);
  }, 20_000);
  ...
  test("6. parse exported dust applications", async () => {
    const data = await Bun.file(`${FIXTURES_DIR}/dust-applications.xls`).arrayBuffer();
    ...
    console.log("  [6] First:", first.applicationId, "|", first.status, "|", first.companyName);
  }, 15_000);
  ...
  test.skip("8. search inspections (server timeout — too many unfiltered results)", async () => {
  ...
  test("10. navigate to Enforcement Action Search", async () => {
    ...
    console.log("  [10] Enforcement Action Search loaded, page:", client.currentPage);
  }, 20_000);

  test.skip("11. search enforcement actions (server timeout — too many unfiltered results)", async () => {
  ...
  test("13. search settlements", async () => {
    ...
    if (tableMatch) console.log("  [13] Results table:", tableMatch[1]);
  }, 60_000);
  ...
  test("15. search site visits", async () => {
    ...
    const tableMatch = html.match(/id="([^"]*(?:Table|table)[^"]*)"/);
    if (tableMatch) console.log("  [15] Results table:", tableMatch[1]);
  }, 60_000);
  ...
  test("16. navigate to Compliance Report Search", async () => {
    ...
    console.log("  [16] Compliance Report Search loaded, page:", client.currentPage);
  }, 20_000);
  ...
  test("17. search compliance reports", async () => {
    ...
    const tableMatch = html.match(/id="([^"]*(?:Table|table)[^"]*)"/);
    if (tableMatch) console.log("  [17] Results table:", tableMatch[1]);
  }, 60_000);
});
