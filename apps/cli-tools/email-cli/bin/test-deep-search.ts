 ...
 *
 * Run with:
 *   bun apps/cli-tools/email-cli/bin/test-deep-search.ts "your search query" "your goal"
 *
 * Example:
 *   bun apps/cli-tools/email-cli/bin/test-deep-search.ts "Villas on McQueen" "Find all contract info for this project"
 *   bun apps/cli-tools/email-cli/bin/test-deep-search.ts "SWPPP" "Find SWPPP-related emails and documents"
 */
...
async function main() {
  ...
  const outputPath = "apps/cli-tools/email-cli/data/inbox/last-search.json";
  const output = {
    query: initialQuery,
    goal,
    userId,
