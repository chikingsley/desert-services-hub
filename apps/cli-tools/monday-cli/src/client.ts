...
// ============================================================================
// Single Item Rich Fetch
// ============================================================================

/**
 * Get a single item with full column data (linked items, mirrors, values).
 * Use this when you need the complete picture for one item.
 */
export async function getItemRich(
  itemId: string
): Promise<MondayItemRich | null> {
  const result = await query<{
    items: Array<RawRichItem & { board: { id: string } }>;
  }>(`
    query {
      items(ids: [${itemId}]) {
        id
 ...
 * Returns public_url which expires in ~1 hour — download immediately.
 */
 ...
 *
 * The target URL must respond to a challenge POST (echo back { challenge }).
 * See docs/reference/monday-webhooks.md for details.
 */
