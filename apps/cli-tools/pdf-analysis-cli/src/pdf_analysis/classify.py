...
_RULES: list[tuple[DocumentType, float, list[str], str]] = [
    ...
    ("insurance", 0.90, [r"Certificate\s+of\s+(?:Liability\s+)?Insurance", r"ACORD"], "ACORD insurance cert"),
    ("insurance", 0.85, [r"Insurance\s+Requirements", r"(?:Workers|General\s+Liability|Umbrella)"], "Insurance requirements"),
    ...
    ("lien_waiver", 0.90, [r"(?:Conditional|Unconditional)\s+(?:Progress|Final)\s+(?:Lien\s+)?Waiver"], "Lien waiver"),
    ...
    ("checklist", 0.90, [r"Check\s*[Ll]ist", r"(?:Required|Received|Approved|Pending)"], "Contract checklist"),
    ...
    ("contract", 0.85, [r"SUBCONTRACT\b", r"Subcontractor\s+(?:agrees|warrants|shall|covenants)"], "Subcontract terms"),
    ("contract", 0.80, [r"(?:^|\n)\s*CONTRACT\b", r"(?:Contractor|Subcontractor)\s+(?:agrees|shall|warrants)"], "Contract with party terms"),
    ...
    # ---- Dust permit ----
    ("dust_permit", 0.90, [r"Dust\s+(?:Control\s+)?Permit", r"(?:Maricopa|MCAQD)"], "Maricopa dust permit"),
...
def classify_pdf(pdf_path: Path) -> ClassifyResult:
    ...
    if not text.strip():
        ...
        if any(k in name_lower for k in ("swppp", "erosion", "sediment")):
            return ClassifyResult("swppp_plan", 0.60, ["no text; filename hint"], page_count, path.name)
        if any(k in name_lower for k in ("grading", "drainage", "civil")):
            return ClassifyResult("civil_plan", 0.60, ["no text; filename hint"], page_count, path.name)
        if any(k in name_lower for k in ("wo ", "work order", "work_order")):
            return ClassifyResult("work_order", 0.60, ["no text; filename hint"], page_count, path.name)
        ...
        if "w-9" in name_lower or "w9" in name_lower:
            return ClassifyResult("tax_form", 0.60, ["no text; filename hint"], page_count, path.name)
    ...
    if not matches:
        ...
        if not matches:
            ...
            contract_signals = sum(
                1
                for keyword in [
                    "shall", "agrees to", "herein", "hereby", "indemnify",
                    "scope of work", "payment terms", "retainage",
                ]
            ...
            if contract_signals >= 3:
                return ClassifyResult(
                    ...
                    page_count, path.name, snippet,
                )
            return ClassifyResult("unknown", 0.30, ["no rules matched"], page_count, path.name, snippet)
    ...
    if best_type not in _PRIMARY_TYPES and page_count > _DEEP_SCAN_THRESHOLD:
        ...
        primary_matches = [(t, c, l) for t, c, l in matches if t in _PRIMARY_TYPES]
        ...
        if not primary_matches:
            ...
            primary_matches = [(t, c, l) for t, c, l in deep_matches if t in _PRIMARY_TYPES]
    ...
    for t, _c, l in matches[1:3]:
        indicators.append(f"also: {t} ({l})")
