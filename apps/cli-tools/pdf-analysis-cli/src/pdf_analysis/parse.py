...
# ---------------------------------------------------------------------------
PLUMBER_PAGE_RE = re.compile(r"--- Page (\d+) ---")
...
class ChunkSpec:
    ...
    plumber_pages: list[PageText]  # includes overlap pages
...
1. **TEXT-LAYER** (pdfplumber) — exact text from the PDF's internal data. \
...
def _split_text_by_pages(text: str, marker_pattern: re.Pattern[str]) -> list[PageText]:
    ...
    Works for both pdfplumber (``--- Page N ---``) and OCR (``<!-- Page N -->``)
    marker styles.
    """
...
def _build_chunks(
    ...
    """
    plumber_by_page = {p.page_num: p for p in plumber_pages}
    ocr_by_page = {p.page_num: p for p in ocr_pages}
    all_pages = sorted(set(plumber_by_page) | set(ocr_by_page))
    ...
    for i, pn in enumerate(all_pages):
        p_len = len(plumber_by_page[pn].text) if pn in plumber_by_page else 0
    ...
    for idx, (start_idx, end_idx) in enumerate(core_ranges):
        ...
        chunks.append(
            ChunkSpec(
                ...
                end_page=core_end,
                plumber_pages=[plumber_by_page[n] for n in chunk_nums if n in plumber_by_page],
...
def _format_reconcile_prompt(
    ...
    if chunk is None:
        ...
        if chunk_index > 0 and chunk.plumber_pages:
            first = min(p.page_num for p in (chunk.plumber_pages or chunk.ocr_pages))
...
async def _reconcile_chunk(
    ...
    prompt = _format_reconcile_prompt(plumber_text, ocr_text, chunk=chunk, chunk_index=chunk_index)
...
async def parse_pdf(
    ...
    # Phase 1: quick text-layer check + pdfplumber extraction
    text_result = extract_text(path)
    pdfplumber_text = text_result.text if text_result.has_text else ""
    ...
    plumber_pages = _split_text_by_pages(pdfplumber_text, PLUMBER_PAGE_RE)
    ocr_pages = _split_text_by_pages(ocr_text, OCR_PAGE_RE)

    # Fallback: if markers weren't found, treat entire text as one page
    if not plumber_pages and pdfplumber_text.strip():
        plumber_pages = [PageText(page_num=1, text=pdfplumber_text)]
    ...
    if len(chunks) <= 1:
        ...
        reconciled, rmodel = await _reconcile_chunk(
            pdfplumber_text, ocr_text, reconcile_model, manager
        )
    else:
        print(
