/**
 * Sync project files to SharePoint
 *
 * NOTE: MinIO storage removed Feb 2026. This script previously downloaded
 * from MinIO and uploaded to SharePoint. Now only creates folder structure.
 *
 * Usage:
 *   bun apps/cli-tools/sharepoint-cli/bin/sync-project-files.ts --notion-id=<id>
 *   bun apps/cli-tools/sharepoint-cli/bin/sync-project-files.ts --project-name="Symbiont"
 *   bun apps/cli-tools/sharepoint-cli/bin/sync-project-files.ts --dry-run --notion-id=<id>
 *
...
if (!(notionId || projectNameQuery)) {
  console.error(
    "Usage: bun apps/cli-tools/sharepoint-cli/bin/sync-project-files.ts --notion-id=<id> --contractor='Weis Builders' --project='The Verge' [--dry-run]"
  );
  console.error(
    "   or: bun apps/cli-tools/sharepoint-cli/bin/sync-project-files.ts --project-name='Symbiont' --contractor='Mead & Hunt' --project='Symbiont' [--dry-run]"
  );
  process.exit(1);
}
...
if (attachments.length === 0) {
  console.log("No PDF attachments found for this project.");
  process.exit(0);
}
...
for (const { attachment, subfolder } of plan) {
  ...
  console.log(
    `  SKIP ${subfolder}/${safeName} — MinIO removed, use SharePoint directly`
  );
  failed++;
}
