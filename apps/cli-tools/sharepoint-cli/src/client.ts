...
export class SharePointClient {
  ...
  /**
   * Get a file or folder metadata on the default drive by path
   */
  async getItemByPath(itemPath: string): Promise<SharePointItem> {
    ...
    const item = await this.client
      .api(`/drives/${driveId}/root:/${itemPath}`)
      .get();
    return this.parseItem(item as Record<string, unknown>);
  }

  /**
   * Move an item by path into a destination folder path on the default drive.
   * Optionally rename during move.
   */
  async moveItem(
    sourcePath: string,
    ...
    const destination = this.isRootPath(destinationFolderPath)
    ...
    return this.parseItem(moved as Record<string, unknown>);
  }
