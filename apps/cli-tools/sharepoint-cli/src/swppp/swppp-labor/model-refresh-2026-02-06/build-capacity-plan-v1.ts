...
function productionDomainCount(
...
function buildMarkdown(
  ...
  const totalBacklogSafe = backlogJobs.reduce(
    (s, r) => s + r.plannedSafeHours,
    0
  );
  const totalBacklogTypical = backlogJobs.reduce(
    (s, r) => s + r.typicalHours,
    0
  );
  ...
  lines.push(
    `- Nominal capacity (man-hours/day): ${round2(nominalCapacityHours)}`
  );
  lines.push(
    `- Safe planning capacity (man-hours/day): ${round2(safeCapacityHours)}`
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "| Date | Jobs | Planned Safe Hours | Typical Hours | Safe Capacity | Overload |"
  );
  lines.push("|---|---:|---:|---:|---:|---:|");
...
function main(): void {
  ...
  const estimates: JobEstimate[] = rows.map((row) => {
    ...
    const segmentedMinutes = isProduction(expanded)
      ? dot(
          PROD_FEATURES,
          expanded,
          model.segmentedModel.productionCoefficients
        )
    ...
    let plannedSafeHours =
      ((Math.max(0, segmentedMinutes / 60) + travelManHours) * riskMultiplier) /
      CONFIG.planningUtilization;
    plannedSafeHours = Math.max(plannedSafeHours, typicalHours * 1.05);

    const specialistRoles = getSpecialistRoles(
      family,
      workDescription,
      expanded
    );
  ...
  const confirmedJobs = estimates.filter(
    (e) => e.worksheet === "Confirmed Schedule"
  );
  const backlogJobs = estimates.filter(
    (e) => e.worksheet === "Need to Schedule"
  );
  ...
  const dailySummaries = [...dailyMap.values()]
    .map((d) => {
      const overloadHours = Math.max(
        0,
        d.totalPlannedSafeHours - safeCapacityHours
      );
      return {
        ...d,
  ...
  console.log(
    `Safe capacity per day (man-hours): ${round2(safeCapacityHours)}`
  );
  console.log(`\nWrote: ${OUTPUT_JSON}`);
