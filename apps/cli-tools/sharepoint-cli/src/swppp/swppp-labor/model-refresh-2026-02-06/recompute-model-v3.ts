...
function productionDomainCount(sample: Sample): number {
  ...
  const fenceScreen =
    ...
    0;
  const sockSilt =
    f.install_sock_lf + f.relocate_sock_lf + f.install_silt_fence_lf > 0;
...
function trainFamilyBundle(train: Sample[]): FamilyBundle {
  ...
  const fallback =
    train.reduce((sum, row) => sum + row.manMinutes, 0) / (train.length || 1);
  return {
    prodW,
    adminW,
    familyWeights,
...
function runWorkFamilyModel(samples: Sample[]): FamilyModelSummary {
  ...
  const holdoutPred = test.map((s) =>
    predictWithFamilyBundle(s, holdoutBundle)
  );
  const holdoutActual = test.map((s) => s.manMinutes);
...
function main(): void {
  ...
  console.log(
    `  P90 APE: ${pct(workFamilyModel.crossValidation.mean.p90Ape)}\n`
  );
