...
/**
 * SWPPP labor model refresh (v3 extractor + multi-family segmentation).
 *
 ...
 *
 * V3 additionally tests finer work-family routing:
 * - fence/screen
 * - sock/silt
 * - inlet-only
 * - rock/trackout
...
const FENCE_SCREEN_FEATURES: ExpandedFeature[] = [
  "install_panels",
  "relocate_panels",
  "remove_panels",
  "install_privacy_screen_lf",
...
const SOCK_SILT_FEATURES: ExpandedFeature[] = [
  "install_sock_lf",
  "relocate_sock_lf",
  "install_silt_fence_lf",
  "maintenance_flag",
...
const INLET_FEATURES: ExpandedFeature[] = [
  "inlets",
  "maintenance_flag",
  "base_setup",
];

const ROCK_TRACKOUT_FEATURES: ExpandedFeature[] = [
  "rock_entrance_count",
  "rumble_grate_count",
  "trackout_rock_tons",
  "maintenance_flag",
...
const MIXED_INSTALL_FEATURES: ExpandedFeature[] = PROD_FEATURES;
...
type WorkFamily = (typeof WORK_FAMILIES)[number];

const FAMILY_FEATURES: Record<WorkFamily, ExpandedFeature[]> = {
  admin_trip: ADMIN_FEATURES,
  fence_screen: FENCE_SCREEN_FEATURES,
  sock_silt: SOCK_SILT_FEATURES,
  inlet: INLET_FEATURES,
  rock_trackout: ROCK_TRACKOUT_FEATURES,
  mixed_install: MIXED_INSTALL_FEATURES,
};

const MIN_FAMILY_TRAIN_SAMPLES = 30;
...
interface FamilyModelSummary {
  sampleCount: number;
  familySampleCounts: Record<WorkFamily, number>;
  holdout: Metrics & { trainCount: number; testCount: number };
  crossValidation: { folds: FoldMetrics[]; mean: Metrics };
  familyCoefficients: Record<WorkFamily, FeatureMap>;
  familyModelMeta: Record<
    WorkFamily,
    {
      trained: boolean;
      trainCount: number;
      minTrainSamples: number;
      fallback: "segment_prod_admin";
    }
  >;
}
...
function parseLaborMinutes(text: string): number {
  ...
  while (match !== null) {
    ...
    match = menRe.exec(text);
  }

  // Additional pattern observed frequently in logs:
  // "(efren was on the tractor for 2 hours)" or "(carlos was doing labor for 30 minutes)"
  const personForDurationRe =
    /\b[a-z][a-z'-]+\b[^.;\n]{0,50}?\bfor\s+(\d+(?:\.\d+)?)\s*(hours?|hrs?|hr|minutes?|mins?|min)\b/gi;
  match = personForDurationRe.exec(text);
  while (match !== null) {
    const duration = parseNum(match[1] ?? "0");
    const unit = (match[2] ?? "").toLowerCase();
    const minutes = duration * (unit.startsWith("h") ? 60 : 1);
    if (Number.isFinite(minutes) && minutes > 0) {
      total += minutes;
    }
    match = personForDurationRe.exec(text);
  ...
  // Fallback for plain "for X hours/minutes" entries without explicit men or person names.
  if (total <= 0) {
    const genericForDurationRe =
      /\bfor\s+(\d+(?:\.\d+)?)\s*(hours?|hrs?|hr|minutes?|mins?|min)\b/gi;
    match = genericForDurationRe.exec(text);
    while (match !== null) {
      const duration = parseNum(match[1] ?? "0");
      const unit = (match[2] ?? "").toLowerCase();
      const minutes = duration * (unit.startsWith("h") ? 60 : 1);
...
function productionDomainCount(sample: Sample): number {
  const f = sample.expanded;
  const fenceScreen =
    f.install_panels +
      f.relocate_panels +
      ...
      f.install_privacy_screen_lf >
    0;
  const sockSilt = f.install_sock_lf + f.relocate_sock_lf + f.install_silt_fence_lf > 0;
  const inlet = f.inlets > 0;
  const rockTrackout =
    f.rock_entrance_count + f.rumble_grate_count + f.trackout_rock_tons > 0;
  return [fenceScreen, sockSilt, inlet, rockTrackout].filter(Boolean).length;
}

function getWorkFamily(sample: Sample): WorkFamily {
  const f = sample.expanded;
  const domains = productionDomainCount(sample);
  if (domains === 0) {
    return "admin_trip";
  }
  if (domains > 1) {
...
function predictOne(
  ...
  for (let i = 0; i < featureNames.length; i++) {
    sum += (sample.expanded[featureNames[i] ?? "base_setup"] ?? 0) * (w[i] ?? 0);
  }
  return sum;
}
...
function runModel(
  ...
  const rows = shuffle(samples, 20260206);
  ...
  for (let fold = 0; fold < k; fold++) {
    ...
    const xTe = foldTest.map((s) => featureNames.map((f) => selector(s)[f] ?? 0));
...
function runSegmentedModel(samples: Sample[]): SegmentedSummary {
  ...
  for (let fold = 0; fold < k; fold++) {
    ...
    const fallback =
      foldTrain.reduce((s, v) => s + v.manMinutes, 0) /
      (foldTrain.length || 1);
...
function makeFamilyRecord<T>(factory: () => T): Record<WorkFamily, T> {
  return {
    admin_trip: factory(),
    fence_screen: factory(),
    sock_silt: factory(),
    inlet: factory(),
    rock_trackout: factory(),
    mixed_install: factory(),
  };
}

function countByFamily(samples: Sample[]): Record<WorkFamily, number> {
  const counts = makeFamilyRecord(() => 0);
  for (const sample of samples) {
    counts[getWorkFamily(sample)] += 1;
  }
  return counts;
}
...
interface FamilyBundle {
  prodW: number[];
  adminW: number[];
  familyWeights: Partial<Record<WorkFamily, number[]>>;
  familyTrainCounts: Record<WorkFamily, number>;
  fallback: number;
}

function trainFamilyBundle(train: Sample[]): FamilyBundle {
  const prodTrain = train.filter((s) => isProductionSample(s));
  const adminTrain = train.filter((s) => !isProductionSample(s));
  const prodXY = buildXY(prodTrain, PROD_FEATURES);
  const adminXY = buildXY(adminTrain, ADMIN_FEATURES);
  const prodW =
    prodXY.x.length > 0
  ...
  const familyTrainCounts = makeFamilyRecord(() => 0);
  const familyWeights: Partial<Record<WorkFamily, number[]>> = {};

  for (const family of WORK_FAMILIES) {
    const familyRows = train.filter((s) => getWorkFamily(s) === family);
    familyTrainCounts[family] = familyRows.length;
    if (familyRows.length < MIN_FAMILY_TRAIN_SAMPLES) {
      continue;
    }
    const features = FAMILY_FEATURES[family];
    const familyXY = buildXY(familyRows, features);
    familyWeights[family] = trainNonNegativeRidge(familyXY.x, familyXY.y);
  }

  const fallback = train.reduce((sum, row) => sum + row.manMinutes, 0) / (train.length || 1);
  return {
    prodW,
    adminW,
    familyWeights,
...
function runWorkFamilyModel(samples: Sample[]): FamilyModelSummary {
  const rows = shuffle(samples, 20260206);
  const split = Math.floor(rows.length * 0.8);
  const train = rows.slice(0, split);
  const test = rows.slice(split);

  const holdoutBundle = trainFamilyBundle(train);
  const holdoutPred = test.map((s) => predictWithFamilyBundle(s, holdoutBundle));
  const holdoutActual = test.map((s) => s.manMinutes);
  const holdout = evaluatePredictions(holdoutActual, holdoutPred);
  ...
  for (let fold = 0; fold < k; fold++) {
    ...
    const foldTest = shuffled.slice(start, end);
    const foldTrain = [...shuffled.slice(0, start), ...shuffled.slice(end)];
    const bundle = trainFamilyBundle(foldTrain);
    const pred = foldTest.map((s) => predictWithFamilyBundle(s, bundle));
    const actual = foldTest.map((s) => s.manMinutes);
    const metrics = evaluatePredictions(actual, pred);
    folds.push({
      fold: fold + 1,
  ...
  const allBundle = trainFamilyBundle(samples);
  const familyCoefficients = makeFamilyRecord<FeatureMap>(() => ({}));
  const familyModelMeta = makeFamilyRecord(() => ({
    trained: false,
    trainCount: 0,
    minTrainSamples: MIN_FAMILY_TRAIN_SAMPLES,
    fallback: "segment_prod_admin" as const,
  }));

  for (const family of WORK_FAMILIES) {
    const features = FAMILY_FEATURES[family];
    const w = allBundle.familyWeights[family];
    const coeff: FeatureMap = {};
    for (let i = 0; i < features.length; i++) {
      coeff[features[i] ?? `f_${i}`] = w?.[i] ?? 0;
    }
    familyCoefficients[family] = coeff;
    familyModelMeta[family] = {
      trained: w !== undefined,
      trainCount: allBundle.familyTrainCounts[family],
      minTrainSamples: MIN_FAMILY_TRAIN_SAMPLES,
      fallback: "segment_prod_admin",
    };
  }
  ...
  return {
    sampleCount: samples.length,
    familySampleCounts: countByFamily(samples),
    holdout: {
      trainCount: train.length,
      testCount: test.length,
      ...holdout,
...
function main(): void {
  ...
  const workFamilyModel = runWorkFamilyModel(expandedSamples);
  ...
  const result = {
    ...
    sampleCounts: {
      ...
      workFamilySamples: workFamilyModel.familySampleCounts,
    },
    legacyModel,
    expandedModel,
    segmentedModel,
  ...
  const outPath = join(OUTPUT_DIR, "swppp-model-refresh-v3.json");
  writeFileSync(outPath, JSON.stringify(result, null, 2));

  const pct = (n: number) => `${(n * 100).toFixed(1)}%`;
  console.log("SWPPP Labor Model Refresh V3");
  console.log("============================\n");
  ...
  console.log(`  Median APE: ${pct(legacyModel.crossValidation.mean.medianApe)}`);
  ...
  console.log(`  Median APE: ${pct(expandedModel.crossValidation.mean.medianApe)}`);
  ...
  console.log(`  P90 APE: ${pct(segmentedModel.crossValidation.mean.p90Ape)}\n`);
  console.log("Work-family routed model (5-fold CV mean):");
  console.log(`  R²: ${pct(workFamilyModel.crossValidation.mean.r2)}`);
  console.log(
    `  Median APE: ${pct(workFamilyModel.crossValidation.mean.medianApe)}`
  );
  console.log(`  P90 APE: ${pct(workFamilyModel.crossValidation.mean.p90Ape)}\n`);
