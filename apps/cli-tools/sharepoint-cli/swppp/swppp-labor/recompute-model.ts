...
function crossValidate(
  ...
  const shuffled = shuffleDeterministic(allSamples, 1_337);
...
function runModel(samples: Sample[]): ModeledSummary {
  const shuffled = shuffleDeterministic(samples, 2_026_02_06);
...
function main(): void {
  ...
  const shuffled = shuffleDeterministic(modeledSamples, 2_026_02_06);
  const coreSamples = modeledSamples.filter((s) => isCoreActivityText(s.text));
  const strictSamples = modeledSamples.filter((s) => isStrictProductionSample(s));
  ...
  console.log(`  Mean median APE: ${pct(coreModel.crossValidation.mean.medianApe)}`);
  console.log(`  Mean p90 APE: ${pct(coreModel.crossValidation.mean.p90Ape)}\n`);
  ...
  console.log(`  Mean p90 APE: ${pct(strictModel.crossValidation.mean.p90Ape)}\n`);
