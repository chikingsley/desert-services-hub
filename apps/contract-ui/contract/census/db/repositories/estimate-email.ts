 ...
 */
export function unlinkEmailFromEstimate(estimateId: number, emailId: number): boolean {
  const result = db.run(
...
export function findEmail(search: string | number) {
  if (typeof search === "number") {
    return db.query<{
      ...
      from_email: string
      received_at: string
    }, [number]>(`SELECT id, subject, from_email, received_at FROM emails WHERE id = ?`).get(search)
  }
