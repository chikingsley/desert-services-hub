 ...
 *
 * Usage:
 *   bun services/contract/census/sync/groups.ts
 *   bun services/contract/census/sync/groups.ts --since=2025-01-01
 *   bun services/contract/census/sync/groups.ts --group=internalcontracts
 *   bun services/contract/census/sync/groups.ts status
 */
    ...
    for (const conv of conversations) {
      ...
      for (const thread of fullConv.threads) {
        for (const post of thread.posts) {
          ...
          // If any sibling email in the same conversation is linked to a project,
          // inherit that project_id for this email
          if (conv.topic) {
            // Use conversation ID (thread ID) for linking
