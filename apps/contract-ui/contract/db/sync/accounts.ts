 ...
 * to hub accounts by domain. This is the bridge that connects emails to
 * Monday — once a hub account has a monday_account_id, you can trace:
 *
 *   Email → Hub Account (by domain) → Monday Contractor → Estimates/Projects
 *
 * Usage:
 *   bun apps/contract-ui/contract/db/sync/accounts.ts
 *   bun apps/contract-ui/contract/db/sync/accounts.ts --dry-run
 *   bun apps/contract-ui/contract/db/sync/accounts.ts --stats
 */
  ...
  do {
    ...
    for (const item of items) {
      ...
      if (rawDomain) {
        ...
        domain = (cleaned && cleaned.length > 0) ? cleaned : null;
      }
...
// ============================================
// Link contractors to hub accounts
// ============================================
...
function linkContractorsToAccounts(
  ...
  for (const contractor of contractors) {
    ...
    if (existing) {
      ...
      // No hub account for this domain — create one
      result.unmatched++;
      console.log(
...
function printStats(): void {
  ...
  console.log(`Total hub accounts:           ${total?.count ?? 0}`);
  ...
  console.log(
    `  With hub account:           ${emailsWithAccount?.count ?? 0}`
  );
  console.log(
...
if (import.meta.main) {
  ...
  try {
    ...
    console.log("Linking to hub accounts...");
