  ...
  try {
    ...
    for (const email of nonSpamEmails) {
      ...
      const emailData: InsertEmailData = {
        messageId: email.id,
        internetMessageId: email.internetMessageId ?? null, // RFC 2822 for cross-mailbox matching
...
if (import.meta.main) {
  ...
  const incremental = !fullSync; // Incremental is now the default
  ...
  try {
    ...
    if (includeGroups) {
      ...
      const { syncAllGroups } = await import("@contract/db/sync/groups");
