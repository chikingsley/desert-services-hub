 ...
 *
 * Syncs estimate data from Monday.com ESTIMATING board to the hub database.
 * Downloads estimate PDFs to MinIO storage.
 *
 * Usage:
 *   bun apps/contract-ui/contract/db/sync/estimates.ts [options]
 *
 * Options:
      ...
      if (skipFiles) {
        ...
        if (pdfAssets.length > 0) {
          ...
          if (existing?.estimateStoragePath) {
            ...
            const uploadResult = asset
              ? await downloadAndUploadEstimatePdf(item.id, asset)
              : null;

            if (asset && uploadResult) {
              updateEstimateStorage(
                item.id,
                uploadResult.bucket,
                uploadResult.path,
        ...
        if (includePlans) {
          ...
          if (plansPdfAssets.length > 0) {
            ...
            // Skip if already synced
            if (planAsset && !existing?.plansStoragePath) {
              try {
...
if (import.meta.main) {
  ...
  syncEstimates({
    ...
    .then((result) => {
      ...
      // Step 2: Link estimates to projects - REMOVED
      // This will be handled by the worker when it syncs to DB
