...
/**
 * Run OCR on a PDF file with Gemini 3 Flash.
 *
 * Strategy:
 * 1. Check if OCR output already exists (cache hit)
 * 2. Try Gemini OCR with retries
 * 3. Save the result to a .md file for future use
 *
 * Note: Gemini 3 Flash provides better accuracy for construction documents
 * compared to traditional OCR engines.
 */
export async function runOCR(pdfPath: string): Promise<string> {
  ...
  // Try Gemini OCR with retries (primary - better for construction documents)
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      console.log(
        `[OCR] Attempting Gemini OCR (attempt ${attempt + 1}/${MAX_RETRIES})...`
      );
      ...
      console.error(
        `[OCR] Gemini attempt ${attempt + 1} failed: ${lastError.message}`
      );
  ...
  throw new Error(`Gemini OCR failed: ${lastError?.message || "unknown"}`);
}
...
export function parseOCRPages(ocrText: string): string[] {
  ...
  for (let i = 1; i < parts.length; i += 2) {
    const rawPageNum = parts[i];
    if (!rawPageNum) continue;
    const pageNum = Number.parseInt(rawPageNum, 10);
    const content = parts[i + 1]?.trim() || "";

    if (content && !Number.isNaN(pageNum)) {
...
Extraction Commands:
  ...
  bun apps/contract-ui/contract/workflow/extract.ts ocr <pdf-path>
    Run OCR on a single PDF

  bun apps/contract-ui/contract/workflow/extract.ts extract <project-folder>
