    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            category: { type: Type.STRING, description: 'Project category e.g. Contract, Permit, Billing, Inquiry' },
            summary: { type: Type.STRING, description: 'One sentence summary' },
            suggestedProject: { type: Type.STRING, description: 'Name of existing project it likely belongs to' },
            nextSteps: {
              type: Type.ARRAY,
...
export const generateTemplateResponse = async (context: string, templateType: string) => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
