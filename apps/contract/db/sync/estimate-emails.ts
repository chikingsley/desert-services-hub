...
// Top-level regex for tokenization
 ...
 */
function processEstimate(estimate: EstimateRow): {
  linked: number;
    ...
    for (const email of domainEmails) {
      ...
      if (matchedToken && byType.name !== undefined) {
        insertStmt.run(
          ...
          email.id,
          "name",
          `domain:${estimate.account_domain},token:${matchedToken}`
        );
        linkedEmailIds.add(email.id);
        byType.name++;
        if (email.conversation_id) {
...
function processBatch(): void {
  ...
  const contractorMap = new Map<
    string,
    Array<{ estimate: EstimateRow; tokens: string[] }>
  >();
  ...
  function normalizeContractor(name: string): string {
    return name
      ...
      .replace(
        /\b(llc|inc|corp|co|ltd|company|companies|construction|builders|building|group|services|contracting)\b/g,
        ""
  ...
  function findMatchingEstimates(
    ...
    for (const [estContractor, estimates] of contractorMap) {
      ...
      if (
        normalizedEmail.includes(estContractor) ||
        estContractor.includes(normalizedEmail)
      ) {
        // Extra check: must share a significant word (not just "co" or "inc")
        const emailWords = normalizedEmail
          .split(" ")
          .filter((w) => w.length > 3);
        const estWords = estContractor.split(" ").filter((w) => w.length > 3);
        const hasCommonWord = emailWords.some((ew) =>
          estWords.some((ew2) => ew === ew2)
        );
        if (hasCommonWord) {
  ...
  for (const email of directEmails) {
    ...
    for (const { estimate, tokens } of matchingEstimates) {
      ...
      insertStmt.run(
        ...
        email.id,
        "direct",
        `domain:${domain},token:${matchedToken}`
      );
      linkedPairs.add(pairKey);
      directMatches++;
...
// Main
const singleEstimate = process.argv
  .find((a) => a.startsWith("--estimate="))
  ?.split("=")[1];
