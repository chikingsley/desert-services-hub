...
for (const item of sampleEstimate.lineItems) {
  console.log(
    `${item.item.padEnd(25)} | ${String(item.qty).padStart(5)} | ${(item.unit || "").padEnd(4)} | $${item.unitCost.toFixed(2).padStart(8)} | $${item.total.toFixed(2).padStart(10)} | ${item.taxable ? "T" : " "} | ${item.section}`
  );
}
