...
### GSQ VEG (Pre-Contract - NTA Only)

- BPR Companies NTA for SWPPP ($17,440)
- **Key learning:** NTA can be email-only (no PDF attachment)
- **Key learning:** SOV comes from estimate when no contract exists yet
- **Key learning:** Scope changes can happen AFTER NTA but BEFORE subcontract
- Filter sock removed after award - need to track email threads for modifications
- Reconciliation shows $4,290 variance - verify when subcontract arrives
- See `gsq-veg/notes.md` for full analysis
...
- **NTA email-only** - some contractors send award as plain email, not PDF
- **Plans not synced** - Monday has separate columns for estimates (`file_mksebs2e`) and plans (`file_mkseqmab`). Only estimates are synced to MinIO. To get plans, query Monday API: `assets(column_ids: ["file_mkseqmab"])` → `public_url`

---

---

## Pre-Contract Research (NTA but no Subcontract)

When you have an estimate and Notice to Award but no signed subcontract yet:
...
### 2. Query hub.db for Project Context

```sql
-- Find estimate in database
SELECT id, name, estimate_number, contractor, bid_status, bid_value
FROM estimates
WHERE estimate_number = '12172501' OR name LIKE '%GSQ VEG%';

-- Find related emails
SELECT id, subject, from_email, received_at, body_preview
FROM emails
WHERE subject LIKE '%GSQ VEG%'
ORDER BY received_at DESC;

-- Find attachments
SELECT a.id, a.name, a.storage_path, e.subject
FROM attachments a
JOIN emails e ON a.email_id = e.id
WHERE a.storage_path IS NOT NULL
  AND e.subject LIKE '%GSQ VEG%';
```

### 3. Find the NTA (Notice to Award)

- Search emails for "Notice of Award", "NTA", "Notice to Proceed"
- **NTA may be email-only** - not all awards have PDF attachments
- Save email content as `NTA-YYYY-MM-DD.txt` if no PDF
...
- **Additional Services**: Optional items (water truck, temp fence, etc.)
- **Total**: Sum of all included items
