  ...
  jQuery.validator.addMethod(
    ...
    function (n, t) {
      ...
      if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)) {
        var r = n.split("/"),
          f = Number.parseInt(r[0], 10),
          e = Number.parseInt(r[1], 10),
          o = Number.parseInt(r[2], 10),
          u = new Date(o, f - 1, e);
        i =
          u.getFullYear() === o && u.getMonth() === f - 1 && u.getDate() === e
            ? !0
            : !1;
      } else {
        i = !1;
...
Planning.BuildingPermitReports = ((n, t) => {
  var i,
    r = [],
    e = () => {
    ...
    o = () => {
      ...
      n.ajax({
        ...
        url:
          ...
          "/BuildingPermit/CheckForMoreReportItems",
    ...
    v = () => {
      ...
      n("td").hide();
      r.forEach((t) => {
        n("th:nth-child(" + t + ")").show();
        n("td:nth-child(" + t + ")").show();
      });
    },
    s = () => {
      i = {
        PageNumber: 0,
        PermitTypeID: n("#permitType").val(),
        StartDate: n("#startDate").val(),
        EndDate: n("#endDate").val(),
    ...
    h = () => {
      (i === null || typeof i == "undefined") && s();
      var f = Number.parseInt(n("#currentPage").val());
      i.PageNumber = f;
      n("#moreBtn").hide();
      n.ajax({
        type: "POST",
        url: ApplicationOptions.BaseUrl + "/BuildingPermit/LoadMoreReportItems",
        data: JSON.stringify(i),
        contentType: "application/json; charset=utf-8",
        dataType: "html",
        success(s) {
          if (
            (n("#searchResults").append(s),
            f === 0 && t.helpers.scrollTo("#search-progress"),
            o(),
            e(),
            n("#moreBtnImg").hide(),
            v(),
            ...
            var h = window.sessionStorage,
              c = JSON.stringify(n("#searchResults").html()),
              l = Number.parseInt(n("#recordCount").val()) || 0;
            l > 0
              ? h.setItem("buildingPermitReportSearchResults", c)
              : h.setItem(
                  "buildingPermitReportSearchResults",
                  JSON.stringify("")
                );
            h.setItem(
              "buildingPermitReportSearchResultsPage",
              Number.parseInt(n("#currentPage").val())
            );
            h.setItem("buildingPermitReportSearchData", JSON.stringify(i));
            h.setItem("buildingPermitReportDisplayList", r);
          }
        },
        error(t, i, r) {
          u();
    ...
    c = () => {
      n(".columnCheckbox").prop("checked", !1);
      n("#box1").prop("checked", !0).prop("disabled", !0);
      n("#box2").prop("checked", !0).prop("disabled", !0);
    },
    y = () => {
      var u = window.sessionStorage,
        f = u.getItem("buildingPermitReportDisplayList"),
        i,
        t;
      for (
        r = f.split(","),
        ...
        n("#box" + r[t]).prop("checked", !0);
      }
    },
    f = () => {
      n("#reportForm").valid() &&
        (l(), n("#searchResults").empty(), n("#currentPage").val(0), s(), h());
    },
    p = () => {
      n("#reportForm").valid() && n("#reportForm").submit();
    },
    w = () => {
      if (
        (n("#currentPage").val(0),
        ...
        t.removeItem("buildingPermitReportSearchResults");
        t.removeItem("buildingPermitReportSearchResultsPage");
        t.removeItem("buildingPermitReportSearchData");
        t.removeItem("buildingPermitReportDisplayList");
      }
    },
    b = () => {
      var t, r, u;
      ...
      n("#permitType,#startDate,#endDate").on("keydown", (n) => {
        n.keyCode === 13 && (n.preventDefault(), f());
      });
      localStorage.reportDetailsScroll = "";
      ...
      Modernizr.sessionstorage &&
        ...
        (r = JSON.parse(t.getItem("buildingPermitReportSearchResults"))),
        r &&
          (n("#searchResults").html(r),
          (u = JSON.parse(t.getItem("buildingPermitReportSearchResultsPage"))),
          n("#currentPage").val(u - 1),
          (i = JSON.parse(t.getItem("buildingPermitReportSearchData"))),
          y(),
          n("#permitType").val(i.PermitTypeID),
          n("#startDate").val(i.StartDate),
          n("#endDate").val(i.EndDate)),
        o(),
        e(),
        n("#moreBtnImg").hide());
      n(".columnCheckbox").on("click", function () {
        n(".columnCheckbox:checked").length > 8 &&
          ((this.checked = !1),
          alert("Please limit the report to a maximum of 8 columns."));
      });
      n.validator.addMethod(
        "later",
        (t, i, r) => {
          var u = n(r[0]).val();
          return /Invalid|NaN/.test(new Date(u)) ||
            /Invalid|NaN/.test(new Date(t))
            ? !0
            : new Date(t) > new Date(u);
        },
      ...
      n.validator.addMethod(
        ...
        (t, i, r) => {
          ...
          return /Invalid|NaN/.test(new Date(u)) ||
            /Invalid|NaN/.test(new Date(t))
            ? !0
            : ((f = 946_944e5), new Date(t) - new Date(u) <= f);
        },
        "Date range must be less than 3 years."
      );
      n("#reportForm").validate({
        onchange: !0,
        errorElement: "div",
        rules: { endDate: { later: ["#startDate"], three: ["#startDate"] } },
      });
    };
  return { Init: b, ReportSearch: f, ReportDownload: p, ReportSearchReset: w };
