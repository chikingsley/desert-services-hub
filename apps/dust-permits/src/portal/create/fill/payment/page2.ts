...
async function readAmounts(page: Page): Promise<{
  ...
  const subTotal = await readText(page, paymentPage2.subTotal);
  const convenienceFee = await readText(page, paymentPage2.convenienceFee);
  const total = await readText(page, paymentPage2.total);
...
async function readCardLastFour(page: Page): Promise<string | null> {
  const mask = await readText(page, paymentPage2.cardMask);
  if (!mask) {
    return null;
  }
...
export async function confirmPayment(
  ...
  // Check TOS checkbox
  const tosResult = await setCheckboxWithSelectors(
    page,
  ...
  const payOk = await clickButton(page, paymentPage2.payNowButton, "Pay Now");
