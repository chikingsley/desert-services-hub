...
const REAL_PARCELS = [
  { address: "22841 N Cave Creek Rd", apn: "213-02-169C", city: "Phoenix" },
  { address: "7191 E Ashler Hills Dr", apn: "216-50-010A", city: "Scottsdale" },
  { address: "25800 N Norterra Pkwy", apn: "210-04-015B", city: "Phoenix" },
  { address: "16155 W Elwood St", apn: "500-06-046B", city: "Goodyear" },
  {
...
describe("queryParcelByCoordinates", () => {
  ...
  test("handles edge case: coordinates at county boundary", async () => {
    ...
    const edgeCoords = { lat: 33, lng: -113.3 };
...
describe("queryParcelByAPN", () => {
  ...
  test("handles APN with different formats", async () => {
    ...
    if (apnResult) {
      ...
      const normalizedOriginal = apn.replaceAll(/[-\s.]/g, "");
      const normalizedResult = apnResult.apn.replaceAll(/[-\s.]/g, "");
  ...
  test("cleans APN format before query", async () => {
    ...
    if (result) {
      expect(result.apn.replaceAll(/[-\s.]/g, "")).toBe(
        apn.replaceAll(/[-\s.]/g, "")
      );
    }
  });
