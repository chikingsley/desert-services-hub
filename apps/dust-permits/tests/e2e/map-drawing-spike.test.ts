    ...
    async () => {
      ...
      const discovery = await esriFrame?.evaluate(() => {
        ...
        try {
          ...
          results["dijit.registry"] = { error: String(error) };
        }

        // Check for global esri namespace (ArcGIS 3.x)
        try {
          const { esri } = window as unknown as Record<string, unknown>;
          if (esri) {
            results["window.esri"] = {
              exists: true,
          ...
          results["window.esri"] = { error: String(error) };
        }
        ...
        try {
          ...
          results.mapContainer = { error: String(error) };
        }
        ...
        try {
          ...
          results["editorWidget.error"] = String(error);
        }

        // Look for jimuMapView (Experience Builder pattern)
        try {
          const { jimuMapView } = window as unknown as Record<
            string,
            {
              view?: { map?: unknown };
              jimuMapView?: unknown;
    ...
    async () => {
      ...
      const conversionResult = await esriFrame?.evaluate(({ lat, lng }) => {
        ...
        if (!dijit?.registry?.byId) {
          return { error: "dijit.registry not available", success: false };
        }
    ...
    async () => {
      ...
      const drawResult = await esriFrame?.evaluate(() => {
        try {
          ...
          if (!center) {
            return { error: "Could not get map center", success: false };
          }
