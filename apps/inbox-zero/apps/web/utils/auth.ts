...
type AuthSession = Awaited<ReturnType<typeof betterAuthConfig.api.getSession>>;

const BYPASS_AUTH_DEFAULT_EMAIL = "owner@inboxzero.local";
...
function isBypassAuthEnabled() {
  return process.env.INBOXZERO_BYPASS_AUTH === "true";
}

async function getBypassAuthSession(): Promise<AuthSession> {
  const bypassEmail = (
    process.env.INBOXZERO_BYPASS_AUTH_EMAIL || BYPASS_AUTH_DEFAULT_EMAIL
  )
    .trim()
    .toLowerCase();
  ...
  const user =
    ...
    (await prisma.user
      .create({
        data: { email: bypassEmail, name: "Bypass User" },
        select: { id: true, email: true, name: true, image: true },
      })
      .catch(async () => {
        // Handle concurrent requests creating the bypass user at the same time.
  ...
  if (!user) {
    throw new Error("Failed to resolve bypass auth user");
  }

  await ensureBypassAppOnlyMailboxes(user.id);
...
async function ensureBypassAppOnlyMailbox({
  userId,
  mailboxEmail,
}: {
  userId: string;
  ...
  const providerAccountId = `app-only:${mailboxEmail}`;
...
function getConfiguredAppOnlyMailboxes() {
  const configured = process.env.INBOXZERO_APP_MAILBOXES?.trim();
  if (configured) {
