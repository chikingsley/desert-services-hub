...
export async function updateAccountSeatsForPremium(
  ...
  logger.info("Skipping external seat sync (billing disabled)", {
    hasStripeItem: Boolean(premium.stripeSubscriptionItemId),
    hasLemonItem: Boolean(premium.lemonSqueezySubscriptionItemId),
    totalSeats,
  });
}
