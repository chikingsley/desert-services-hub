...
#
# Usage:
#   bun run docker:local:run                 # run with docker-compose (recommended)
#   bun run docker:local:run --standalone    # run image only (use external DB/Redis)
...
if ! docker image inspect "${FULL_IMAGE}:latest" > /dev/null 2>&1; then
  ...
  echo "Build first with: bun run docker:local:build"
  exit 1
fi
