...
import { resolveEmailToEstimate } from "@lib/email-resolution/resolve-estimate";
import { resolveEmailToProject } from "@lib/email-resolution/resolve-project";
...
async function runSparkEstimateTieBreaker(
  ...
  const payload = {
    email: {
      ...
      attachmentNames: ambiguous.matchResult.context.attachmentNames.slice(0, 12),
      fromDomain: ambiguous.matchResult.context.fromDomain,
      projectHints: ambiguous.matchResult.context.projectHints.slice(0, 12),
      contractorHints: ambiguous.matchResult.context.contractorHints.slice(0, 12),
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (!warnedOpencodeUnavailable && message.toLowerCase().includes("enoent")) {
