...
async function cleanupTestRows(): Promise<void> {
  const estimateRows = (await db
    .query<{ id: number }>(
      "SELECT id FROM estimates WHERE monday_item_id LIKE ?"
    )
      ...
      ? ((await db
          .query<{ project_id: number }>(
            `SELECT DISTINCT project_id
             ...
             WHERE estimate_id IN (${estimateIds.map(() => "?").join(", ")})`
          )
  ...
  const projectRowsByName = (await db
    .query<{ id: number }>(
      "SELECT id FROM projects WHERE seed_source = 'estimate_sync' AND name LIKE ?"
    )
  ...
  if (estimateIds.length > 0) {
    await db.run(
      `DELETE FROM estimates
       WHERE id IN (${estimateIds.map(() => "?").join(", ")})`,
      estimateIds
    );
  }
...
describe("project seed sync integration", () => {
  test("deduplicates prefixed estimate variants into one seed and promotes to active", async () => {
    ...
    const project = (await db
      .query<{ lifecycle_state: string; seed_key: string | null }>(
        "SELECT lifecycle_state, seed_key FROM projects WHERE id = ?"
      )
      .get(projectId)) as {
  ...
  test("marks stale seed projects as lost", async () => {
    ...
    await db.run(
      "UPDATE projects SET lifecycle_state = 'seed', last_evidence_at = now() - interval '90 days' WHERE id = ?",
      [row.project_id]
    );
