...
const selectNextJob = db.query<WebhookJob>(`
  ...
  ORDER BY
    CASE
      ...
      WHEN job_type = 'email_resolve_backfill' THEN 2
      ELSE 3
    END,
    created_at ASC
  LIMIT 1
...
const enqueueEmailResolveIfNotQueued = db.prepare(`
  ...
  WHERE NOT EXISTS (
    SELECT 1
    FROM webhook_jobs
    WHERE job_type IN ('email_resolve', 'email_resolve_backfill')
      AND status IN ('pending', 'processing')
...
export async function dequeue(): Promise<WebhookJob | null> {
  ...
  return { ...job, attempts: job.attempts + 1, status: "processing" };
}
...
export async function enqueueEstimateFileSweep(
  ...
  if (ids.length === 0) {
    return { batched: 0, queued: 0, total: 0 };
  }
  ...
  return { batched: batch.length, queued, total: ids.length };
}
