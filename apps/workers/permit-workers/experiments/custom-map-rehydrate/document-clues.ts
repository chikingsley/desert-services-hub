...
export async function buildDocumentCluesForPermit(
  ...
  if (permit.projectId && permit.projectId > 0) {
    if (cache?.has(permit.projectId)) {
      docs = cache.get(permit.projectId) ?? [];
      debug.push(`documents from cache: ${docs.length}`);
    } else {
      const fetchLimit = Math.max(maxDocs * 5, 30);
      const fetched = await fetchProjectDocuments(
        permit.projectId,
        container,
        fetchLimit
      );
      const ranked = rankDocumentsForPermit(fetched, permit, maxDocs);
      docs = ranked.map((r) => r.row);
      cache?.set(permit.projectId, docs);

      debug.push(`documents queried: ${fetched.length}`);
  ...
  const coordinates = combinedText ? extractCoordinates(combinedText) : null;
