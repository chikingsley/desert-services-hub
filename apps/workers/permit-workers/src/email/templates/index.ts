...
export function getSignature(options: SignatureConfig): string {
  let sig = `<div>Best,</div>
<div>--</div>
<div><br></div>
<div>${options.name}</div>
<div>${options.title}</div>
<div>E: <a href="mailto:${options.email}">${options.email}</a></div>`;

  if (options.phone) {
    sig += `\n<div>M: ${options.phone}</div>`;
  }
...
/**
 * Wrap a plain body with signature and logo
 *
 ...
 *
 * @example
 * const html = wrapWithSignature("Hello, this is my message.");
