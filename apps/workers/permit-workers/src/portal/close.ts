...
export async function fillClosePermitDialog(
  ...
  try {
    ...
    console.log(`    ✗ Failed to fill reason: ${e}`);
    return result;
  }
  await sleep(SETTLE_MS);
...
export async function closePermit(
  ...
  if (!atSearch) {
    return { success: false, error: "Failed to navigate to search" };
  }
  ...
  if (!opened) {
    return { success: false, error: `Permit ${permitId} not found` };
  }
  ...
  const popup = await clickClosePermitButton(page, context);
  if (!popup) {
    return { success: false, error: "Failed to open close dialog" };
  }
  ...
  return { success: false, error: "Failed to confirm close" };
}
