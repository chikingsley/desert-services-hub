...
CRO - used for scheduling

Rolloff -> wendy for hours
PJ ->

checking if work got done -> if done, send invoice to customer
...
__look into the billing side of CRO -> might be able to export from CRO to quickbooks

can pull a report weekly from cro

theyve changed things since we started CRO - can look at teh billing side.

pull report from rolloff once a week, bill as much as possible weekly, we have customers where all will be done in a month.

one offs will be invoiced immediately if possible - for rolloffs

RO, PJ

1/ wk ro -> manually input invoices into QB
for pj, have a list that are done regularly monthly,
one offs - bill immediately

- --

try and chase people when things (AR - )
...
calls with questions - drivers would call kerin directly.
answers clients from answering service -> 15-20 calls/day
...
og - contract comes through with signs, if does, need noi or dust permit (certain requiremnts for signage)

info is scattered -> contracts go out to everyone, and individuals have to figure out what's necessary

fed by contract/estimate work
...
Opportunity --> Billing through CRO could be setup. Rolloffs are a simpler process, PJs more difficult.
