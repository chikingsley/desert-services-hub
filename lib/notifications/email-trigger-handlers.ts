...
import { getAttachmentsForEmail } from "@lib/db/repositories/attachment";
  ...
  try {
    const row = await db
      .query<{ facility_id: string | null }, [string]>(
        "SELECT facility_id FROM dust_permits_filed_by_desert_services WHERE id = $1"
      )
      .get(previousAppId);
...
export async function handlePaymentEmail(
  ...
  // DISABLED: auto-draft creation scattered drafts across recipient mailboxes
...
async function findPermitByInvoice(
  ...
  const row = await db
    .query<Record<string, unknown>, [string]>(
      "SELECT * FROM dust_permits_filed_by_desert_services WHERE invoice_number = $1"
    )
    .get(invoiceNumber);
