...
_RULES: list[KeywordRule] = [
    ...
    KeywordRule(
        ...
        "warning",
        re.compile(
            r"\b(?:fines|penalty|penalties|citation|citations|liable|liability|indemnif(?:y|ication)|hold harmless|back\s*charge)\b",
            re.IGNORECASE,
        ),
...
_SCOPE_CREEP_TERM_RE = re.compile(
    r"\b(?:maintain|maintenance|repair|remove|removal|amend|adjust|replace|replacement|upgrade|upgrading|regrade|replenish)\b",
    re.IGNORECASE,
)
_SCOPE_CREEP_EXCLUSION_RE = re.compile(
    r"\b(?:maintenance manuals?|record drawings?|equitable adjustment|adjustment theories|maintain(?:ing)?\s+schedule|prompt payment)\b",
    re.IGNORECASE,
)

_INSPECTION_MENTION_RE = re.compile(r"\binspection(?:s)?\b", re.IGNORECASE)
_INSPECTION_QUANTITY_RE = re.compile(
    r"\b(?:at\s+least\s+)?(?:[2-9]|\d{2,3}|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred)\s+inspections\b",
    re.IGNORECASE,
)
...
def _scan_scope_creep(text: str, result: ScanResult) -> None:
    """Find likely scope-creep obligations with light context filtering."""
    for match in _SCOPE_CREEP_TERM_RE.finditer(text):
        start = max(0, match.start() - 180)
        end = min(len(text), match.end() + 180)
        context = text[start:end]
        if _SCOPE_CREEP_EXCLUSION_RE.search(context):
            continue
        ...
        _add_flag(
            ...
            explanation="Scope creep term detected",
        )


def _first_near_match(
    text: str,
    *,
    pattern: re.Pattern[str],
    ...
    for candidate in pattern.finditer(text):
        ...
        for anchor in anchors:
            if c_end < anchor.start() - max_distance:
                continue
            if c_start > anchor.end() + max_distance:
                continue
            return candidate
    return None
...
def _scan_inspections(text: str, result: ScanResult) -> None:
    ...
    quantity_match = _first_near_match(
        text,
        pattern=_INSPECTION_QUANTITY_RE,
        anchors=inspection_mentions,
        max_distance=180,
    )
    frequency_match = _first_near_match(
        text,
        pattern=_INSPECTION_FREQUENCY_RE,
        anchors=inspection_mentions,
        max_distance=220,
    )
    rain_match = _first_near_match(
        text,
        pattern=_INSPECTION_RAIN_TRIGGER_RE,
        anchors=inspection_mentions,
        max_distance=260,
    )

    has_quantity = quantity_match is not None
    has_frequency = frequency_match is not None
    has_rain_trigger = rain_match is not None
    ...
    if not has_quantity:
        ...
        if quantity_match is None:
            raise RuntimeError("quantity_match unexpectedly missing when has_quantity is true")
        _add_flag(
            result,
            rule_id="INSPECTION_QUANTITY_FOUND",
            severity="info",
            keyword=quantity_match.group(0),
            text=text,
    ...
    if not has_frequency:
        ...
        if frequency_match is None:
            raise RuntimeError("frequency_match unexpectedly missing when has_frequency is true")
        _add_flag(
            result,
            rule_id="INSPECTION_FREQUENCY_FOUND",
            severity="info",
            keyword=frequency_match.group(0),
            text=text,
            start=frequency_match.start(),
            end=frequency_match.end(),
    ...
    if not has_rain_trigger:
        ...
        if rain_match is None:
            raise RuntimeError("rain_match unexpectedly missing when has_rain_trigger is true")
        _add_flag(
            result,
