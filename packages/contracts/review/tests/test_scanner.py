...
class TestInspectionCoverage:
    ...
    def test_section_number_is_not_quantity(self) -> None:
        text = (
            "8.1 Inspection of Work. Subcontractor shall provide access for inspection by Owner. "
            "No quantity commitment is stated."
        )
        result = scan(text)
        assert len(_active(result, "INSPECTION_QUANTITY_FOUND")) == 0
        assert len(_active(result, "INSPECTION_QUANTITY_MISSING")) == 1

    def test_frequency_far_from_inspection_does_not_count(self) -> None:
        text = (
            "Inspection of work may occur at any time by Owner representatives. "
            + ("filler text " * 120)
            + "Subcontractor shall furnish reports daily, weekly, and monthly as requested."
        )
        result = scan(text)
        assert len(_active(result, "INSPECTION_FREQUENCY_FOUND")) == 0
        assert len(_active(result, "INSPECTION_FREQUENCY_MISSING")) == 1
...
class TestLiabilityWarnings:
    ...
    def test_fine_grading_does_not_trigger_liability(self) -> None:
        text = "SITE-1020 SE CY: Fine Grading 5 01-Sep-27 08-Sep-27"
        result = scan(text)
        flags = _active(result, "LIABILITY_OR_FINES_LANGUAGE")
        assert len(flags) == 0
