...
def _find_header_index(header_cells: list[str], labels: set[str]) -> int | None:
    normalized_labels = {_normalize_header_label(label) for label in labels}
    for index, cell in enumerate(header_cells):
        if _normalize_header_label(cell) in normalized_labels:
            return index
    return None
...
def _extract_header(pages: list[pdfplumber.page.Page]) -> EstimateHeader:
    ...
    estimator_labels = {"Estimator", "Sales Rep", "Sales Representative"}
    estimate_number_labels = {"Estimate #", "Estimate#", "Estimate Number"}
    normalized_estimator_labels = {
    ...
    for table in tables:
        ...
        # Date / Estimate # can appear in a dedicated two-column table.
        date_idx = _find_header_index(header_cells, {"Date"})
        if date_idx is not None and date_idx < len(data_row) and not date:
            date = _cell(data_row[date_idx])

        estimate_idx = _find_header_index(header_cells, estimate_number_labels)
        if estimate_idx is not None and estimate_idx < len(data_row) and not estimate_number:
            est_raw = _cell(data_row[estimate_idx])
        ...
        # Estimator may be labeled as "Estimator" or "Sales Rep".
        estimator_idx = _find_header_index(header_cells, estimator_labels)
        if estimator_idx is not None and not estimator:
            for candidate_row in table[1:4]:
                if estimator_idx >= len(candidate_row):
                    continue
                candidate = _cell(candidate_row[estimator_idx])
                if (
                    candidate
                    and _normalize_header_label(candidate)
                    not in normalized_estimator_labels
                ):
                    estimator = candidate
                    break
