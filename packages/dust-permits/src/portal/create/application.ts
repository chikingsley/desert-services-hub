...
export async function createNewCompanyApplication(
  ...
  if (!popup) {
    return { applicationId: null, error: "No popup", success: false };
  }
  if (!(await handleNewAppPopup(popup, { copyFromApp, flow: "new-company" }))) {
    return { applicationId: null, error: "Popup failed", success: false };
  }
  await sleep(SETTLE_MS);
  if (!(await waitForApplicationCreated(page))) {
    return { applicationId: null, error: "Creation timed out", success: false };
  }
  return { applicationId: await getCreatedApplicationId(page), success: true };
}
