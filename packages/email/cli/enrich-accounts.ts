 ...
 *
 * Run: bun packages/email/cli/enrich-accounts.ts
 * Check status: bun packages/email/cli/enrich-accounts.ts --status
 *
 * Free tier: 10,000/month - we have 3,580 accounts = ~36% of limit
 * Runtime: ~6 hours for all accounts (rate limited)
 */

import { cleanCompany } from "@enrichment/pdl/company";
...
Usage:
  bun packages/email/cli/enrich-accounts.ts           # Run enrichment (all pending)
  bun packages/email/cli/enrich-accounts.ts --status  # Check progress
  bun packages/email/cli/enrich-accounts.ts --batch 100  # Process 100 accounts
  bun packages/email/cli/enrich-accounts.ts --test    # Test with 3 accounts
