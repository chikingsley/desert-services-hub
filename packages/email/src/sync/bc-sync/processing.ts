...
const getUnprocessedAttachmentsForEmail = db.query<
...
export async function processBcFilesForEmail(
  emailId: number
): Promise<BuildingConnectedSyncResult> {
  const startedAt = Date.now();
  ...
  const attachments = await getUnprocessedAttachmentsForEmail.all(emailId);
  ...
  // Filter attachments to ensure they actually match BC signals.
  // This acts as a guard since we bypass the WHERE clause from the batch query.
  const validAttachments = attachments.filter(hasBuildingConnectedSignal);
  ...
  console.log(
    `${LOG} Processing ${validAttachments.length} BuildingConnected attachments for email ${emailId} (concurrency: ${CONCURRENCY})`
  );

  await mkdir(BACKFILL_DIR, { recursive: true });
  const client = createGraphClient();

  for (let i = 0; i < validAttachments.length; i += CONCURRENCY) {
    const chunk = validAttachments.slice(i, i + CONCURRENCY);
    ...
    const outcomes = await Promise.allSettled(
      chunk.map(async (att) => {
        try {
          ...
          throw new Error(
            `Failed to process attachment ${att.attachment_id_pk}: ${
    ...
    for (const outcome of outcomes) {
      if (outcome.status === "rejected") {
        result.failed++;
        const errStr = String(outcome.reason);
        result.errors.push(errStr);
        console.error(`${LOG} ${errStr}`);
      } else {
  ...
  result.elapsedMs = Date.now() - startedAt;
  const minutes = result.elapsedMs / 1000 / 60;
  result.attachmentsPerMinute = minutes > 0 ? result.processed / minutes : 0;
