...
export async function runBodyLinkBackfill(options: BodyLinkBackfillOptions): Promise<void> {
  let query = `
  ...
  `;
  const params: any[] = [options.since.toISOString()];
  ...
  if (candidates.length === 0) {
    console.log(`[worker] Body link backfill: no candidates found since ${options.since.toISOString()}`);
    return;
  }
  ...
  for (const email of candidates) {
    try {
      ...
      failures++;
      console.error(`[worker] Body link backfill failed for email ${email.id}:`, err);
    }
  }

  console.log(`[worker] Body link backfill: processed=${processed}, skipped=${skipped}, linksFound=${linksFound}, attachmentsInserted=${attachmentsInserted}, failures=${failures}`);
}
