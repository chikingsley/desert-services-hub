...
```bash
bun packages/monday/cli/cli.ts audit-rel all --active-only
...
| Script | Purpose |
| -------- | --------- |
| `cli/sync-estimates.ts` | Main sync (use `--dry-run` first!) |
...
bun packages/monday/cli/sync-estimates.ts --dry-run --limit=100

# Check results, then real run
bun packages/monday/cli/sync-estimates.ts --limit=500

# Full sync (takes time)
bun packages/monday/cli/sync-estimates.ts
```

## Key Files

1. `cli/sync-estimates.ts` - Main sync logic with account resolution, TF consolidation, status folder moves
2. `src/client.ts` - Monday API with retry logic, `getItemsRich()`, `getItemNames()` batch lookup
3. `../sharepoint/src/client.ts` - SharePoint Graph API client
