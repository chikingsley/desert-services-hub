 ...
 *
 * NOTE: Binary object-storage copy is disabled. This script now only creates
 * SharePoint folder structure.
 *
 * Usage:
 *   bun packages/sharepoint/cli/sync-project-files.ts --notion-id=<id>
 *   bun packages/sharepoint/cli/sync-project-files.ts --project-name="Symbiont"
 *   bun packages/sharepoint/cli/sync-project-files.ts --dry-run --notion-id=<id>
 *
...
if (!(notionId || projectNameQuery)) {
  console.error(
    "Usage: bun packages/sharepoint/cli/sync-project-files.ts --notion-id=<id> --contractor='Weis Builders' --project='The Verge' [--dry-run]"
  );
  console.error(
    "   or: bun packages/sharepoint/cli/sync-project-files.ts --project-name='Symbiont' --contractor='Mead & Hunt' --project='Symbiont' [--dry-run]"
  );
  process.exit(1);
}
...
// Classify each file
const plan: { attachment: AttachmentRow; subfolder: string }[] = [];
