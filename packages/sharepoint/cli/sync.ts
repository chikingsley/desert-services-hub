...
/**
 * SWPPP Master Poller — Poll Loop
 *
 * Polls SharePoint SWPPP Master Excel and upserts into Postgres swppp_work_orders.
 *
 * Usage:
...
async function run(): Promise<void> {
  console.log(`[SWPPP Master Poller] ${new Date().toISOString()}`);
  ...
  try {
    ...
    console.error("[SWPPP Master Poller] Error:", err);
  }
}

console.log(
  `[SWPPP Master Poller] Starting. Interval: ${interval}ms, Once: ${once}`
);
...
if (once) {
  ...
  while (true) {
    await run();
    console.log(
      `[SWPPP Master Poller] Next sync at ${new Date(Date.now() + interval).toISOString()}\n`
    );
