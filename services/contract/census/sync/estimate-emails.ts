 ...
 * 
 * Run: bun services/contract/census/sync/estimate-emails.ts
 * Or:  bun services/contract/census/sync/estimate-emails.ts --estimate=10142559
 */
...
function processBatch(): void {
  ...
  for (const est of estimates) {
    ...
    const tokens = extractTokens(est.name)
  ...
  for (let i = 0; i < platformEmails.length; i++) {
    ...
    const contractorKey = normalizeContractor(email.real_sender_company)
    
    const matchingEstimates = contractorMap.get(contractorKey)
...
if (singleEstimate) {
  // Process single estimate (keep old method for testing)
