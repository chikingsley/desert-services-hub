...
async function downloadEstimates() {
  ...
  // Store estimates in services/monday/estimates/ directory
  const outputDir = join(import.meta.dir, "..", "estimates");
  ...
  console.log(`🚀 Starting download of ALL estimates to ${outputDir}...`);

  // Download ALL estimates (no limit)
  while (true) {
    const cursorParam: string = cursor ? `, cursor: "${cursor}"` : "";
    const response: MondayItemsPageResponse =
      await query<MondayItemsPageResponse>(`
      query {
    ...
    const itemsPage:
      | MondayItemsPageResponse["boards"][0]["items_page"]
      | undefined = response.boards[0]?.items_page;
    if (!itemsPage) {
      break;
    }

    const itemsWithFiles = itemsPage.items.filter((item: BatchItem) => {
    ...
    // Process all items in the batch (no limit)
