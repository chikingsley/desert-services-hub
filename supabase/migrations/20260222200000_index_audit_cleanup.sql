...
--
-- NOTE: CONCURRENTLY removed so this works inside Supabase migration pipeline.
-- On a fresh DB these indexes mostly don't exist, so IF EXISTS handles it.
-- ============================================================================
...
DROP INDEX IF EXISTS idx_documents_email_attachment_upsert;
...
DROP INDEX IF EXISTS idx_documents_outlook_att;
...
DROP INDEX IF EXISTS idx_documents_monday_asset_id;
...
DROP INDEX IF EXISTS idx_documents_monday_item_id;
...
DROP INDEX IF EXISTS idx_contacts_monday;
...
DROP INDEX IF EXISTS idx_estimates_monday_id;
...
CREATE INDEX IF NOT EXISTS idx_project_match_reviews_account
  ON project_match_reviews (account_id_hint);

-- contract_packets.estimate_id → estimates (3.3K rows)
CREATE INDEX IF NOT EXISTS idx_contract_packets_estimate
...
DROP INDEX IF EXISTS idx_emails_active_dedup_partition;
...
DROP INDEX IF EXISTS idx_emails_platform_name;
...
DROP INDEX IF EXISTS idx_emails_rules_evaluated;
...
DROP INDEX IF EXISTS idx_email_list_dedup_mv_received;       -- 11 MB
DROP INDEX IF EXISTS idx_email_list_dedup_mv_id;             -- 7.8 MB
DROP INDEX IF EXISTS idx_email_list_dedup_mv_from_domain;    -- 2.6 MB
DROP INDEX IF EXISTS idx_email_list_dedup_mv_classification; -- 2.4 MB

-- project_email_dedup_mv (18K rows, all indexes 0 scans)
DROP INDEX IF EXISTS idx_project_email_dedup_mv_project_last;  -- 544 KB
DROP INDEX IF EXISTS idx_project_email_dedup_mv_permit_signal; -- 88 KB
