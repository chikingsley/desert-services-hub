...
export async function recordPageElements(
  ...
  const result = await page.evaluate(
    (opts) => {
      ...
      function getLabel(element: HTMLElement): string | undefined {
        ...
        if (labelledBy) {
          const labelEl = document.querySelector(`#${labelledBy}`);
          if (labelEl) {
      ...
      if (filterSelector) {
        try {
          ...
          allElements = allElements.filter(
            (el) => filteredSet.has(el) || filtered.some((f) => f.contains(el))
          );
        } catch {
  ...
  if (outputPath) {
    ...
    await writeFile(fullPath, JSON.stringify(outputData, null, 2), "utf8");
    savedPath = fullPath;
  }
