 ...
 *
 * Usage: bun run tests/eval/permits/seed-dataset.ts [--count 200]
 */
import { db } from "@lib/db/client";
import { writeFileSync } from "node:fs";
import { join } from "node:path";

const DIR = import.meta.dirname ?? join(process.cwd(), "tests/eval/permits");
...
async function main() {
  ...
  console.log("\nDone. Run eval with: bun run tests/eval/permits/eval.ts --strategy fts");
  process.exit(0);
}
