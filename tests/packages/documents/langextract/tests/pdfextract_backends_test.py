...
from pdfextract.ocr_backends import RapidOcrBackend, available_ocr_backends, create_ocr_backend
...
def _build_test_pixmap() -> pymupdf.Pixmap:
    doc = pymupdf.open()
    page = doc.new_page()
...
def test_rapidocr_prefers_txts_and_boxes_shape():
    backend = RapidOcrBackend()
    pixmap = _build_test_pixmap()

    class Result:
        txts = ("Hello", "World")
        boxes = (
            ((10, 10), (50, 10), (50, 20), (10, 20)),
            ((60, 10), (110, 10), (110, 20), (60, 20)),
        )
        word_results: tuple[Any, ...] = ()

    with patch.object(backend, "_get_engine", return_value=lambda *_args, **_kwargs: Result()):
        out = backend.extract_page(pixmap=pixmap, dpi=72, prompt="ignored")

    assert out.text == "Hello\nWorld\n"
    assert len(out.words) == 2
    assert out.words[0].text == "Hello"
    assert out.words[0].x0 == 10.0
    assert out.words[0].x1 == 50.0
    assert out.words[1].text == "World"
    assert out.words[1].char_start == 6


def test_rapidocr_falls_back_to_word_results_shape():
    backend = RapidOcrBackend()
    pixmap = _build_test_pixmap()
    ...
    class Result:
        ...
        word_results = (
            (
                ("foo", 0.99, ((10, 10), (30, 10), (30, 20), (10, 20))),
                ("bar", 0.88, ((40, 10), (70, 10), (70, 20), (40, 20))),
            ),
        )
    ...
    assert len(out.words) == 2
    assert out.words[0].text == "foo"
    assert out.words[1].text == "bar"
