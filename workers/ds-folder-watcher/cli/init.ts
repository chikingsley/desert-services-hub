...
import { findProjectByName } from "@/workers/ds-folder-watcher/lib/projects.ts";
import {
  addTrackedFolder,
  getTrackedFolders,
  openStateDb,
...
for (const folder of projectFolders) {
  const existing = findProjectByName(folder.displayName);
  ...
  if (existing) {
    console.log(
      `  + ${folder.displayName} → project #${existing.projectNumber}`
    );
    matched++;
  } else {
...
console.log(`\nRun 'bun cli/watch.ts --once' to start polling for changes.`);
