 ...
 * 1. New folders under Projects/Active/ → auto-creates projects
...
import {
  ...
  type MessageChange,
  messagesDelta,
} from "@/workers/ds-folder-watcher/lib/graph.ts";
import {
  ensureHubProject,
  linkMessages,
...
function handleNewFolder(folder: FolderChange): void {
  ...
  if (created) {
    console.log(`  → Created project #${projectNumber}: ${folder.displayName}`);
  } else {
    console.log(`  → Linked to existing project #${projectNumber}`);
  }
...
function handleNewMessages(
  ...
  const hubProjectId = ensureHubProject(folderName);
...
async function poll(): Promise<void> {
  ...
  for (const folder of tracked) {
    try {
      ...
      if (added.length > 0) {
        handleNewMessages(folder.display_name, folder.folder_id, added);
      }
